...
sql_update = '''update ServerPorts set port_number=443 where servers_id in (select servers_id from ServerProjects where projects_id in (select id from Projects where proj_name="Project3" )) and servers_id in (select id from Servers where servertypes_id in (select id from ServerTypes where type_name="apache"));'''
